        ...
        <div className="space-y-4">
          ...
          {questions.length === 0 && (
            <div className="text-center py-8 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg">
              ...
              <p className="text-sm">
                Click "Add Question" to create role-specific questions
              </p>
            </div>
          )}
                        ...
                        >
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              />
                              <Label
                                htmlFor={`requires-input-${index}-${optionIndex}`}
                              >
                                <div className="flex items-center gap-1">
                                  <FileText className="h-3 w-3" />
                                  Show input field when applicant selects this
                                  option
                                </div>
                              </Label>
                            </div>
                            {option.requiresInput && (
                              <div className="ml-6 pt-2 border-t border-gray-200">
                                <Label className="text-sm text-gray-600">
                                  Placeholder text for applicant input
                                </Label>
                                <p className="text-xs text-gray-500 mb-2">
                                  This placeholder will guide applicants on what
                                  to enter when they select this option
                                </p>
                                <Input
...
export default function AdminPositionsPage() {
  ...
  const [createPositionMutation, { loading: createLoading }] = useMutation(
    CREATE_POSITION,
    {
  ...
  const [deletePositionMutation, { loading: deleteLoading }] = useMutation(
    DELETE_POSITION,
    {
      refetchQueries: [GET_POSITION],
  ...
  const handleSelectPosition = useCallback(
    (positionId: string, checked: boolean) => {
  ...
  const handleSelectAll = useCallback(() => {
    setSelectedPositions(
      new Set(currentPositions.map((p: { id: string }) => p.id))
    );
  }, [currentPositions]);
  ...
  const confirmBulkDelete = useCallback(async () => {
    ...
    try {
      // Delete positions one by one (you might want to create a bulk delete mutation)
      const deletePromises = Array.from(selectedPositions).map((id) =>
        deletePositionMutation({ variables: { id } })
      );

      await Promise.all(deletePromises);

      toast.success(
        `Successfully deleted ${selectedPositions.size} position${
  ...
  const handleCreateQuestionsChange = useCallback(
  ...
  const handleEditQuestionsChange = useCallback(
    (newQuestions: Omit<Question, "id">[]) => {
  ...
  const normalizedCreateQuestions: Omit<Question, "id">[] = createQuestions.map(
    (q) => ({
      type: q.type,
      label: q.label,
      placeholder: q.placeholder,
  ...
  const normalizedEditQuestions: Omit<Question, "id">[] = editQuestions.map(
    (q) => ({
      type: q.type,
      label: q.label,
      placeholder: q.placeholder,
  ...
  const createFormProps = useMemo(
    ...
    [
      createFormData,
      normalizedCreateQuestions,
      handleCreateFormDataChange,
      handleCreateQuestionsChange,
    ]
  );
  ...
  const editFormProps = useMemo(
    ...
    [
      editFormData,
      normalizedEditQuestions,
      handleEditFormDataChange,
      handleEditQuestionsChange,
    ]
  );
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            Create and manage job positions with custom questions
          </p>
        </div>
        <div className="flex items-center gap-4">
          <Dialog
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {positions.filter((p: Position) => p.isActive).length}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {positions.reduce(
                    (sum: number, p: Position) => sum + p.questions.length,
                    0
                  )}
                </p>
              </div>
                    ...
                    <TableHead className="w-12">
                      <Checkbox
                        checked={
                          ...
                          currentPositions.every((p: { id: string }) =>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {selectedPositions.size !== 1 ? "s" : ""}? This action cannot be
              undone and will permanently remove the selected positions and all
          ...
          <AlertDialogFooter>
            ...
            >
              {isBulkDeleting ? (
                ...
                `Delete ${selectedPositions.size} Position${
                  selectedPositions.size !== 1 ? "s" : ""
                }`
              )}
            </AlertDialogAction>
